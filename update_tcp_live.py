...
tk.Button(root, text="Start as Server", width=20, command=lambda: root.after(0, ask_for_server_details)).grid(row=2, column=0, pady=5)

# Connect to server button - now uses after() to call ask_for_client_details
tk.Button(root, text="Connect to Server", width=20, command=lambda: root.after(0, ask_for_client_details)).grid(row=2, column=1, pady=5)
